function [coilData, tagData]=coilData_process(direct,folder,cidx,VIEW, outputFolder)
% inputs: 
% direct: folder of coil data file
...
for i=3:-1:1
    for ci=cidx
        ...
        coilData.sig_sync{ci}(i,:)=signDecode(coilData.amp_sync{ci}(i,:), phDiff,outputFolder,VIEW,i);
        
        
        diff_tmp=coilData.ref_phase_1k{i}(i,:)- coilData.phase_1k{ci}(i,:);
        phDiff=min(abs([-2*pi+diff_tmp; diff_tmp; 2*pi+diff_tmp]));
        coilData.sig_1k{ci}(i,:)=signDecode(coilData.amp_1k{ci}(i,:), phDiff,outputFolder,VIEW,i);
    end
end
...
coilName={'leftEye','rightEye','helmetSide','helmetBack'};
    if VIEW
        ...
        for ci=cidx
            ...
            saveas(gcf,[outputFolder coilName{ci} '_coil_sync' '.png'])
            saveas(gcf,[outputFolder coilName{ci} '_coil_sync'], 'epsc')
            saveas(gcf,[outputFolder coilName{ci} '_coil_sync' '.fig'])
        end
        ...
        for ci=cidx
            ...
            saveas(gcf,[outputFolder coilName{ci} '_coil_1k' '.png'])
            saveas(gcf,[outputFolder coilName{ci} '_coil_1k'], 'epsc')
            saveas(gcf,[outputFolder coilName{ci} '_coil_1k' '.fig'])
        end
    end
end