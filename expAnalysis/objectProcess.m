...
data2plot{1}=helmet.pos-mean(helmet.pos(:,trialDur),2); data2plot{2}=helmet.q; data2plot{3}=headRest.pos-mean(headRest.pos(:,trialDur),2);
taggingPlot(tagData.t_sync,data2plot, {'helmet position (m)','helmet rotation', 'headRest position (m)'},tagData,...
...
plot_each_frame(headRest,timings.eyeProbe.trial(1), params.opti2roomTransMatrix)
...
confirmFlag=input('Is the order of markers correct (the extra marker should be the last one)?  Y/N: ','s');
if strcmp(confirmFlag,'N')
...
%% eyeProbe
plot_each_frame(eyeProbe,timings.eyeProbe.trial(1),params.opti2roomTransMatrix);
...
%% table
plot_each_frame( table, 1E4,params.opti2roomTransMatrix);
...
%% table grid
plot_each_frame( tableGrid, 1E4,params.opti2roomTransMatrix);
